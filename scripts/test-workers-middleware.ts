...
export async function runMiddlewareTests(): Promise<void> {
  ...
  const redirectRequest = new Request('https://test.pages.dev/some-path?param=value');
  const redirectResponse = await handleRequest(redirectRequest, mockEnv);
  
  if (redirectResponse.status === 301 && 
      redirectResponse.headers.get('location') === 'https://midnight480.com/some-path?param=value') {
  ...
  const hasAllHeaders = Object.entries(SECURITY_HEADERS).every(([key, value]) => 
    securityResponse.headers.get(key) === value
  );
  ...
  if (cacheResponse.headers.get('Cache-Control') === 'public, max-age=31536000') {
...
async function runDetailedTests(): Promise<void> {
  ...
  try {
    ...
    expect(response.headers.get('location')).toBe('https://midnight480.com/some-path?param=value');
  ...
  try {
    ...
    const request = new Request('https://example.pages.dev/posts/test?utm_source=github');
    const response = await handleRequest(request, mockEnv);
    
    expect(response.status).toBe(301);
    expect(response.headers.get('location')).toBe('https://midnight480.com/posts/test?utm_source=github');
