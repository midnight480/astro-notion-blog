 ...
 * robots.txtの動的生成をテスト
 */
export function validateRobotsTxtGeneration(
  domain: string
...
export function runComprehensiveSeoValidation(urls: string[]): {
  ...
  const totalRecommendations = canonicalResults.flatMap(
    (r) => r.recommendations
  ).length
  ...
  // 全体スコア計算
  const avgCanonicalScore =
    canonicalResults.length > 0
      ? canonicalResults.reduce((sum, r) => sum + r.score, 0) /
        canonicalResults.length
      : 0

  const robotsScore =
    (robotsResults.filter((r) => r.isValid).length /
      Math.max(robotsResults.length, 1)) *
    100
  const redirectScore =
    (redirectResults.filter((r) => r.isValid).length /
      Math.max(redirectResults.length, 1)) *
    100
