...
このプロジェクトでは、統合されたCI/CDワークフローでコード品質を自動チェックしています。

#### 開発者向け手動コマンド

変更をコミットする前に、以下のコマンドでコード品質を確認してください：
...
`deploy-workers.yml`ワークフローが自動的に以下を実行します：
1. 環境変数の検証
2. リントチェック（`npm run lint`）
3. フォーマットチェック（`npm run format:check`）
4. アプリケーションのビルド
5. 全テストの実行
6. Cloudflare Workersへのデプロイ（ステージング/本番）

いずれかのステップが失敗した場合、デプロイは自動的にキャンセルされます。
